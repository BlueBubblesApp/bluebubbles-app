...
class _ThemingColorOptionsListState extends State<ThemingColorOptionsList> {
              ...
              onPressed: () {
                if (ThemeStruct.findOne(controller.text) != null || controller.text.isEmpty) {
                  ...
                  final tuple = Platform.isWindows
                      ? applyWindowsAccent(currentTheme.data, currentTheme.data)
  ...
  Widget build(BuildContext context) {
    ...
    final Rx<Color> _headerColor =
        (SettingsManager().settings.windowEffect.value == WindowEffect.disabled ? headerColor : Colors.transparent).obs;
          ...
          SliverToBoxAdapter(
            child: Obx(() => SettingsOptions<ThemeStruct>(
                  ...
                  options: allThemes.where((a) => !a.name.contains("🌙") && !a.name.contains("☀")).toList()
                    ...
                    ..addAll(allThemes.where((a) => !widget.isDarkMode ? a.name.contains("🌙") : a.name.contains("☀"))),
                  backgroundColor:
                      SettingsManager().settings.skin.value == Skins.Material ? tileColor : _headerColor.value,
                  secondaryColor:
                      SettingsManager().settings.skin.value == Skins.Material ? _headerColor.value : tileColor,
                  ...
                  onChanged: (value) async {
                    ...
                    if (value.name == "Music Theme ☀" || value.name == "Music Theme 🌙") {
                      ...
                      SettingsManager().saveSelectedTheme(context,
                          selectedLightTheme: allThemes.firstWhere((element) => element.name == "Music Theme ☀"),
                          selectedDarkTheme: allThemes.firstWhere((element) => element.name == "Music Theme 🌙"));
                    } else if (currentTheme.name == "Music Theme ☀" || currentTheme.name == "Music Theme 🌙") {
                      if (!widget.isDarkMode) {
                        ThemeStruct previousDark = revertToPreviousDarkTheme();
                        SettingsManager()
                            .saveSelectedTheme(context, selectedLightTheme: value, selectedDarkTheme: previousDark);
                      } else {
                        ThemeStruct previousLight = revertToPreviousLightTheme();
                        SettingsManager()
                            .saveSelectedTheme(context, selectedLightTheme: previousLight, selectedDarkTheme: value);
                      }
