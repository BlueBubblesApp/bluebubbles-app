...
class ActionHandler extends GetxService {
  ...
  Future<void> sendMessage(Chat c, Message m, ReactionType? r) async {
    ...
    if (r == null) {
      ...
      http.sendTapback(c.guid, m.text ?? "", m.guid!, describeEnum(r)).then((response) async {
        final newMessage = Message.fromMap(response.data['data']);
  ...
  Future<void> handleNewMessage(Chat c, Message m) async {
    ...
    c = isParticipantEvent(m) ? await handleNewOrUpdatedChat(c) : (Chat.findOne(guid: c.guid) ?? await handleNewOrUpdatedChat(c));
