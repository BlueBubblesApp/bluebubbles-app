...
class BulkSyncChats extends AsyncTask<List<dynamic>, List<Chat>> {
  ...
  FutureOr<List<Chat>> run() {
    return db.runInTransaction(TxMode.write, () {
      ...
      final handleQuery = db.handles.query(Handle_.uniqueAddressAndService.oneOf(inputHandleAddressesAndServices)).build();
      ...
      db.handles.putMany(inputHandles);

      // 2. Fetch all inserted/existing handles based on input
      final handleQuery2 = db.handles.query(Handle_.uniqueAddressAndService.oneOf(inputHandleAddressesAndServices)).build();
      ...
      // 4. Check for existing chats and save new ones
      final chatQuery = db.chats.query(Chat_.guid.oneOf(inputChatGuids)).build();
      ...
      db.chats.putMany(inputChats);

      // 5. Fetch all inserted/existing chats based on input
      final chatQuery2 = db.chats.query(Chat_.guid.oneOf(inputChatGuids)).build();
...
class SyncLastMessages extends AsyncTask<List<dynamic>, List<Chat>> {
  ...
  FutureOr<List<Chat>> run() {
    return db.runInTransaction(TxMode.write, () {
      ...
      final chatQuery = db.chats.query(Chat_.guid.oneOf(inputGuids)).build();
      ...
      for (int i in chatIds) {
        ...
        final latestMsgQuery = (db.messages.query(Message_.chat.equals(i))..order(Message_.dateCreated, flags: Order.descending)).build();
      ...
      if (updatedChats.isNotEmpty) {
        db.chats.putMany(updatedChats, mode: PutMode.update);
      }
