...
class Attachment {
  ...
  static void bulkSave(Map<Message, List<Attachment>> map) {
    return db.runInTransaction(TxMode.write, () {
      ...
      try {
        ...
        final ids = db.attachments.putMany(attachments);
  ...
  static Attachment? findOne(String guid) {
    ...
    final query = db.attachments.query(Attachment_.guid.equals(guid)).build();
    query.limit = 1;
  ...
  static List<Attachment> find({Condition<Attachment>? cond}) {
    final query = db.attachments.query(cond).build();
    return query.find();
  }
  ...
  static void delete(String guid) {
    ...
    db.runInTransaction(TxMode.write, () {
      ...
      query.close();
      if (result?.id != null) {
        db.attachments.remove(result!.id!);
      }
    });
  }
