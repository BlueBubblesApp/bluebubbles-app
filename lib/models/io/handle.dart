...
class Handle {
  ...
  static List<Handle> bulkSave(List<Handle> handles, {matchOnOriginalROWID = false}) {
    db.runInTransaction(TxMode.write, () {
      ...
      List<int> insertedIds = db.handles.putMany(handles);
  ...
  static Handle? findOne({int? id, int? originalROWID, Tuple2<String, String>? addressAndService}) {
    ...
    if (id != null) {
      final handle = db.handles.get(id) ?? Handle.findOne(originalROWID: id);
      return handle;
    } else if (originalROWID != null) {
      final query = db.handles.query(Handle_.originalROWID.equals(originalROWID)).build();
      query.limit = 1;
      final result = query.findFirst();
      query.close();
      return result;
    } else if (addressAndService != null) {
      final query = db.handles.query(Handle_.address.equals(addressAndService.item1) & Handle_.service.equals(addressAndService.item2)).build();
      query.limit = 1;
      final result = query.findFirst();
      query.close();
      return result;
  ...
  static List<Handle> find({Condition<Handle>? cond}) {
    final query = db.handles.query(cond).build();
    return query.find();
  }
