...
class ThemeStruct {
  ...
  static ThemeStruct getLightTheme() {
    ...
    final query = db.themes.query(ThemeStruct_.name.equals(name ?? "Bright White")).build();
    query.limit = 1;
  ...
  static ThemeStruct getDarkTheme() {
    ...
    final query = db.themes.query(ThemeStruct_.name.equals(name ?? "OLED Dark")).build();
    query.limit = 1;
    final result = query.findFirst();
    if (result == null) {
  ...
  static ThemeStruct? findOne(String name) {
    ...
    return db.runInTransaction(TxMode.read, () {
      final query = db.themes.query(ThemeStruct_.name.equals(name)).build();
      query.limit = 1;
      final result = query.findFirst();
      query.close();
      return result;
  ...
  static List<ThemeStruct> getThemes() {
    ...
    List<ThemeStruct> allThemes = db.themes.getAll();
    // sometimes the theme box is empty, this ensures it is never empty when queried
    if (allThemes.isEmpty) db.themes.putMany(ts.defaultThemes);
    allThemes = db.themes.getAll();
    return allThemes;
  }
