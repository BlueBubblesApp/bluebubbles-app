...
class Box<T> {
  ...
  QueryBuilder<T> query([dynamic qc]) => QueryBuilder<T>();
}
...
class QueryBuilder<T> {
  ...
  QueryBuilder<T> link(dynamic property, [dynamic condition]) => this;
  Query<T> build() => Query<T>();
  Stream<Query<T>> watch() => Stream.empty();
}
