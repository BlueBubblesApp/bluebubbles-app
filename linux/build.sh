...
tmp=$(mktemp)
chmod 644 "$tmp"
jq '.version = "1.12.102.0"' build/linux/$folder/release/bundle/data/flutter_assets/version.json > "$tmp" && mv "$tmp" build/linux/$folder/release/bundle/data/flutter_assets/version.json
