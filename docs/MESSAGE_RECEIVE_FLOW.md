...
Also gated: the `typing-indicator` socket event in `action_handler.dart`, and the "Attach recent photo" smart reply chip (`internalSmartReplies['attach-recent']`) added/removed in `messages_view.dart`, since both live in rows that sit between the list and the text field and feed the same offset calculation.
