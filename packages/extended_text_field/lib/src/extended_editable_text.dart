...
typedef SelectionChangedCallback = void Function(TextSelection selection, dynamic cause);
...
class ExtendedEditableTextState extends State<ExtendedEditableText>
  ...
  bool get supportSpecialText =>
      widget.specialTextSpanBuilder != null && !widget.obscureText && _textDirection == TextDirection.ltr;
  ...
  TextEditingValue _handleSpecialTextSpan(TextEditingValue value) {
    if (supportSpecialText) {
      ...
      if (textChanged) {
        final TextSpan newTextSpan = widget.specialTextSpanBuilder.build(value?.text);
        if (newTextSpan == null) {
          return value;
        }

        final TextSpan oldTextSpan = widget.specialTextSpanBuilder.build(_value?.text);
        value = handleSpecialTextSpanDelete(value, _value, oldTextSpan, _textInputConnection);
  ...
  void _handleSelectionChanged(TextSelection selection, dynamic cause) {
    ...
    if (renderEditable?.handleSpecialText ?? false) {
      final TextEditingValue value =
          correctCaretOffset(_value, renderEditable?.text, _textInputConnection, newSelection: selection);
...
//        _selectionOverlay.showHandles();
    ...
    if (!textChanged && widget.onSelectionChanged != null) widget.onSelectionChanged(selection, cause);
  }
  ...
  void _didChangeTextEditingValue() {
    final bool textChanged = _value?.text != _lastFormattedUnmodifiedTextEditingValue?.text;
    //https://github.com/flutter/flutter/issues/36048
    if (textChanged) {
      _hideSelectionOverlayIfNeeded();
    }
  ...
  ExtendedRenderEditable get renderEditable => _editableKey.currentContext.findRenderObject() as ExtendedRenderEditable;
  ...
  bool showToolbar() {
    ...
    if (_selectionOverlay == null && FocusScope.of(context).focusedChild == widget.focusNode) {
  ...
  @override
  void hideToolbar([bool anything = false]) {
    _selectionOverlay?.hide();
  }
      ...
      child: Scrollable(
        ...
        viewportBuilder: (BuildContext context, ViewportOffset offset) {
          if (offset != null && offset is ScrollPosition) {
            if (offset.minScrollExtent != null && offset.maxScrollExtent != null) {
              // pixels should >= minScrollExtent
              // pixels should <= maxScrollExtent
              offset.correctPixels(offset.pixels.clamp(offset.minScrollExtent, offset.maxScrollExtent) as double);
            }
          }
  ...
  InlineSpan _buildTextSpan() {
    ...
    if (supportSpecialText) {
      final TextSpan specialTextSpan = widget.specialTextSpanBuilder?.build(_value.text, textStyle: widget.style);
...
class _Editable extends MultiChildRenderObjectWidget {
  ...
  @override
  void updateRenderObject(BuildContext context, ExtendedRenderEditable renderObject) {
    renderObject
      ..supportSpecialText = supportSpecialText
      ..text = textSpan
      ..cursorColor = cursorColor
